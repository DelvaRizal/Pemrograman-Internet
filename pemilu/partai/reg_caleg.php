								...
								if (move_uploaded_file($_FILES['image']['tmp_name'], $gambar)) {             
									...
									try{
										...
										$sql = 'insert into caleg value 
												(:username, :password, :nama_lengkap, :tempat_lahir, :tanggal_lahir, :alamat, :email, :website, :partai, :dapil, :visi, :foto)';
										$ssql = $db->prepare($sql);
										$ssql->execute(array(':username' => $_POST['username'], ':password' => md5($_POST['pass']), 
															...
															':partai' => $_POST['Partai'], ':dapil' => $_POST['Daerah'], ':visi' => $_POST['visi'], ':foto' => $gambar));
